...
const ChatDetail = ({navigation, route}) => {
  ...
  const loadMoreMessages = async () => {
    ...
    try {
      ...
      setHasMoreMessages(false); // Nếu có lỗi, dừng việc tải thêm
    } finally {
    ...
    return (
      <View style={{marginBottom: 10}}>
        ...
        {item.text.trim() !== '' && (
          <View
            style={
              item.msgByUserId === userId ? styles.sender : styles.receiver
            }>
      ...
      <View style={styles.container}>
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            `${item._id}${index} chat in chatDetail`
          }
          ListFooterComponent={
        ...
        <View style={styles.row}>
          ...
          <TouchableOpacity onPress={handSendImages}>
            <Image
              source={require('../../assets/icons/chose_icon.png')}
              style={styles.send}
            />
          </TouchableOpacity>
