  ...
  const validateField = (field, value) => {
    ...
    switch (field) {
      case 'email': {
        ...
        error = emailRegex.test(trimmedValue)
          ? ''
      ...
      case 'password': {
        error =
          trimmedValue.length < 8 ? 'Mật khẩu phải có ít nhất 8 ký tự' : '';
        setPasswordError(error);
        return error;
      }
      ...
      {isContinue && (
        <Animated.View style={{marginTop: 10, opacity: fadePasswordAnim}}>
          ...
          <TextField
            ...
            placeholder={
              isRegister ? 'Yêu cầu tối thiểu 8 ký tự' : 'Nhập mật khẩu của bạn'
            }
            onChangeText={text => {
              setPassword(text);
              clearErrorByField('password');
            }}
